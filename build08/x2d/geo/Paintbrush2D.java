...
/**
 * Paintbrush provides an easy to use utility to draw figures and polygons.  Calling its <code>nextStroke</code> method will add the specified coordinate
 * to the object's list, and can be immediately drawn onto the canvas if the boolean passed is true.<br>
 * The width and height parameters in the constructors should be the size of the component of which you are painting on, while the Graphics parameter is the Graphics
 * object the component uses in its <code>paint</code> method.<br>
 * Paintbrush will allow you to draw arbitrary, open figures, however, they are not able to be treated as objects and cannot be specifically redrawn/undrawn.
 * It will also allow you to create a Polygon from your figure which will automatically close the first and last points with a straight line.
 * The <code>drawPolygon</code> method will return the Polygon object you have created as well as draw/fill the shape on screen.<br><br>
 * <code>
...
public class Paintbrush2D extends FreeDraw2D {
     ...
     * @param draw if true, the line will be drawn onto the canvas immediately.
	 * @see #drawStrokes()
	 */
	...
	/**
	 * Clears the currently stored coordinates list and sets it to the passed x and y point arrays.  Each position will correspond to its equivalent in the opposite array.  i.e. (xpoints[0],ypoints[0]) is a point.
	 * @param xpoints An array of x points corresponding to an array of y points.
	 * @param ypoints An array of y points corresponding to an array of x points.
	 * @throws GeoException
	 */
	
	public void setPoints(int[] xpoints, int[] ypoints) throws GeoException {
		if(xpoints.length != ypoints.length) throw(new GeoException("Invalid argument: arrays must be equal in size"));
		else if(xpoints.length == 0 || ypoints.length == 0) throw(new GeoException("Invalid argument: array cannot be empty"));
		else {
			coords.clear();
	...
	public Polygon drawPolygon(boolean fill) throws GeoException {
		if(coords.size() == 0) throw(new GeoException("Empty coordinate data."));
		else {
			...
			if(fill) canvas.fillPolygon(p);
			else canvas.drawPolygon(p);
			return p;
		}
	}
	
	/**
	 * Removes the last set point from the coordinate list.  This will NOT undraw anything from the canvas.
	 */
	 ...
	 * @throws GeoException if argument is less than zero or greater than the current array capacity.
	 */
	
	public int[] getStrokeAt(int index) throws GeoException {
		if(index < 0) throw(new GeoException("Illegal argument: index cannot be less than zero"));
		else if(index > (coords.size()-1)) throw(new GeoException("Illegal argument: requested index exceeds the array capacity.  Size = "+coords.size()));
		else return coords.get(index);
	}
	...
	/**
	 * @return <code>true</code> if the internal coordinate ArrayList is empty.  <code>false</code> otherwise.
	 */
