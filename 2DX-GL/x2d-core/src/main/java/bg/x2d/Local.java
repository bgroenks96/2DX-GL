 ...
 * 
 * This class also deals with how 2DX loads and handles native libraries. By
 * default, 2DX loads native code from the natives JAR file in the classpath.
 * Inside of this JAR, each platform specific directory must exist in order to
 * be counted as supported. Any platforms which do not have an existing
 * directory within the native library location will not be listed as supported
 * by the 2DX-GL software distribution.
 * 
 * The native library location shouldn't change, but a setter method is provided
 * in case it must be changed programmatically. Changes to the default library
 * location should be made to the default value of the 'nativeLib' member of
 * this class.
 * 
...
public abstract class Local {
    ...
    /**
     * Checks to see if the current Java Runtime's version is at or higher than
     * the given minimum. The two arguments stand for the two parts of the
     * version ID (excluding version updates). i.e. Java 1.7 or higher can be
     * checked with <code>minJavaVersion(1,7)</code>.
     * 
     * @param v1
     ...
     */
    public static boolean minJavaVersion(final int v1, final int v2) {
     ...
     * 
     * @return a String value representing the architecture: JVM_X64 constant if
     *         64-bit, JVM_X86 constant otherwise.
     */
    ...
    /**
     * Attempts to determine the human-readable identifier of the graphics
     * device installed on this machine. Usually this method returns the name of
     * the installed video hardware responsible for handling VGA output. The
     * exact model/name returned may vary depending on the underlying OS.
     * 
     * @throws UnsatisfiedLinkError
     *             if 2DX does not support native libraries on the current
     *             platform
     */
    public static native String getGraphicsDevice() throws UnsatisfiedLinkError;

    /**
     * Fetches the amount of RAM the system has available. As opposed to the
     * built in Java functions, this method obtains the amount of free RAM
     * system-wide, outside of the virtual machine.
     * 
    ...
    /**
     * Fetches the total amount of RAM installed on the system. As opposed to
     * the built in Java functions, this method obtains the total amount of RAM
     * installed on the hardware. The accuracy of the result may vary depending
     * on the platform.
     * 
    ...
    /**
     * 
     * @return the file suffix for shared libraries on the native platform (no
     *         '.') or "lib" if unknown.
     */
    ...
    /**
     * Sets the location on the classpath from which native libraries should be
     * loaded from. This method automatically calls checkNativeSupport after
     * setting the new variable.
     * 
     * @param pathLoc
     *            the name of the package from which native code should be
     *            loaded
     * @return true if successful, false otherwise
     */
    public static boolean setNativeLibraryLocation(final String pathLoc) {
    ...
    /**
     * Returns the current platform's directory name for native libraries. Used
     * both by 2DX native libraries and third party native dependencies.
     * 
     * @return
     */
     ...
     * This method is sort of a hack, as it utilizes Java Reflections to set an
     * field used internally in the java.lang.ClassLoader system class. This
     * allows the original path variable loaded at runtime to be changed, as
     * well as setting the external system property.
     * 
     * @param s
     * @throws IOException
     */
    public static void addToLibPath(final String s) throws IOException {
    ...
    /**
     * Loads the specified library into the system. The native library is
     * located via the system class loader and written to a temporary file
    ...
    private static synchronized void checkNativeSupport() {
        ...
        for (int i = 0; i < SYS_COUNT; i++ ) {
            ...
            case 8:
                ...
                if (getPlatform().toLowerCase().contains("solaris") && getJavaArch().equals(JVM_X86)) {
                    current = platform;
                }
                break;
            case 9:
                platform = NATIVE_SOLARIS64;
                if (getPlatform().toLowerCase().contains("solaris") && getJavaArch().equals(JVM_X64)) {
                    current = platform;
                }
            }
