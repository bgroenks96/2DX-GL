...
public class Paintbrush2D extends FreeDraw2D {
    ...
    public void nextStroke(final int x, final int y, final boolean draw, final Graphics canvas) {
    ...
    public void setPoints(final int[] xpoints, final int[] ypoints) throws GeoException {

        if (xpoints.length != ypoints.length) {
            throw (new GeoException("Invalid argument: arrays must be equal in size"));
    ...
    public void drawStrokes(final Graphics canvas) {
    ...
    public int[] getStrokeAt(final int index) throws GeoException {
    ...
    /**
     * Clears this Paintbrush object's coordinate/stroke data. Does nothing to
     * the screen.
     * 
     * @see #isCleared()
     */
