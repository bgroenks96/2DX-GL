...
/**
 * Provides a Map that maps multiple values to a single key. This map is backed
...
public class Multimap<K, V> extends AbstractMap <K, V> {
    ...
    public synchronized V get(final Object key, final int index) {
    ...
    public synchronized V[] getAll(final Object key) {
    ...
    /**
     * @return a Set with all Multimap values COPIED into it. Changes will not
     *         be reflected.
     */
    @Override
    ...
    /**
     * @return a Set with all Multimap keys COPIED into it. Changes will not be
     *         reflected.
     */
    @Override
    ...
    private int findValueIndex(final V val, final V[] vset) {
    ...
    /*
     * Appends the given object onto the existing array by allocating a new
     * array with +1 size.
     */
    private V[] append(final V[] varr, final V val) {
    ...
    /*
     * private void trimArrays() { int nulls = 0; for(K k:keys) if(k == null)
     * nulls++; for(int i=0;i<keys.length;i++) { if(keys[i] == null) { if(i+1 <
     * keys.length) { int ii = 1; while(i + ii < keys.length && keys[i + ++ii]
     ...
     * ii < vals.length) { vals[i] = vals[i + ii]; vals[i + ii] = null; } } }
     * else { int nullCount = 0; V[] varr = vals[i]; for(V v:varr) if(v == null)
     * nullCount++; for(int ii=0;ii<varr.length;ii++) { if(varr[ii] == null) {
     * if(ii+1 < varr.length) { int iii = 0; while(ii + ++iii < varr.length &&
