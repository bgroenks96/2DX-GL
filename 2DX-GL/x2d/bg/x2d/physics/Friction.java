...
public class Friction extends Force {
     ...
     * @param g
     */
    public Friction(final float staticCoeff, final float kineticCoeff, final Gravity g) {
     ...
     * @param g
     */
    public Friction(final double staticCoeff, final double kineticCoeff, final Gravity g) {
     ...
     */
    public Vector2f applyTo(final float time, final float mass, Vector2f forceSum, Vector2f vec) {
        ...
        if (forceSum.getMagnitude() > sx || !FloatMath.equals(vec.getMagnitude(), 0)) {
            ...
            // set the force vector's magnitude according to kinetic friction
            // and angle according to
            // the current
            // velocity. Adding or subtracting pi doesn't technically matter,
            // but for cleanliness we
            // should try
        ...
        /*
         * boolean noForceSum = (forceSum == null); if(noForceSum) forceSum =
         ...
         * !FloatMath.equals(vec.x, 0)) { vecf.x = fkx * mass * fg *
         * -Math.signum(vec.x); } else vecf.x = fneg.x;
         * 
         * float sy = Math.abs(fsy * mass * fg); if(Math.abs(forceSum.y) > sy ||
         * !FloatMath.equals(vec.y, 0)) vecf.y = fky * mass * fg *
         * -Math.signum(vec.y); else vecf.y = fneg.y;
         * 
         * float signX = Math.signum(vec.x); float signY = Math.signum(vec.y);
         * vec = super.applyTo(time, mass, (noForceSum) ? null:forceSum, vec);
         * if(Math.signum(vec.x) != signX) vec.x = 0; if(Math.signum(vec.y) !=
         * signY) vec.y = 0; return vec;
         */
    }
     ...
     * 
     */
    public Vector2d applyTo(final double time, final double mass, Vector2d forceSum, Vector2d vec) {
