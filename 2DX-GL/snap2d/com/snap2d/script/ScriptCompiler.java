...
class ScriptCompiler {
    ...
    public Multimap <String, Function> precompile(final ArrayList <ConstantInitializer> constList,
            final String... scripts) throws ScriptCompilationException {
        ...
        for (String script : scripts) {
            ...
            for (Function f : farr) {
                if (!Character.isUpperCase(f.getName().charAt(0))) {
                    printWarning(f, "script function names should be camel-case (i.e. MyFunction)");
                }
    ...
    private Function[] parseScript(final String src, final ArrayList <ConstantInitializer> clist)
        ...
        for (int i = 0; i < chars.length; i++ ) {
            ...
            if (Character.isWhitespace(c) && buff.length() == 0) {
                ...
                // nextFlush rule exception for PC_CONST flag (single line const
                // initializers):
                ...
                if (ekey == Flags.PC_FUNC) {
                    ...
                    int paramCount = (paramString.isEmpty()) ? 0
                            : paramString.split(Keyword.SEPARATOR.getSymbol()).length;
                    ...
                    if (funcEnd < 0) {
                        throw (new ScriptCompilationException("reached end of script with unterminated function", src,
                                funcStart));
                    }
                    i = funcEnd;

                    Function func = new Function(fname, rtype, params, paramNames,
                            src.substring(funcStart + 1, funcEnd), funcStart + 1);
                    ...
                    if (en < 0) {
                        throw (new ScriptCompilationException(
                                "reached end of script with unterminated constant initializer", src, i + 1));
                    }
    ...
    private void parseParams(final Keyword[] storeParams, final String[] storeNames, final String paramStr,
            final String src, final int pos) throws ScriptCompilationException {
    ...
    // start value is incremented, so the given start position should be the
    // position of the block's starting character
    private int findBlockEnd(final char[] src, int start) {
    ...
    private int findArgEnd(final char[] src, int start) {
    ...
    private final HashMap <String, Variable> paramVars = new HashMap <String, Variable>();
    private HashMap <String, Variable> stackVars = new HashMap <String, Variable>();
    private final HashMap <String, Variable> constVars = new HashMap <String, Variable>();
     ...
     *            before function compilation
     * @throws ScriptCompilationException
     *             if an error occurs during compilation
     */
    public void compile(final Multimap <String, Function> functions, final ArrayList <ConstantInitializer> constList)
            throws ScriptCompilationException {
        ...
        try {
            for (ConstantInitializer cfunc : constList) {
                ...
                for (int ii = 0; ii < count; ii++ ) {
                    ...
                    if (assign < 0) {
                        throw (new ScriptCompilationException(
                                "expected assignment operator '=' in constant declaration", csrc, pos));
                    }
                    String left = dec.substring(0, assign).trim();
                    String[] pts = left.split("\\s+");
                    Keyword type = Keyword.getFromSymbol(pts[0]);
                    if (pts.length < 2) {
                        throw (new ScriptCompilationException("expected identifier after constant type declaration",
                                csrc, pos));
                    }
                    parseVarFromType(type, true, csrc, pos + pts[0].length());

                    pos = en + 1;
                    for (; pos < csrc.length() && Character.isWhitespace(csrc.charAt(pos)); pos++ ) {
                        ; // skip whitespace
                    }
                }
    ...
    private void parseMain(final String src, final int pos) throws ScriptCompilationException {
        ...
        nextDelims.add(Keyword.PARAM_BEGIN);
        nextDelims.add(Keyword.END);
        nextDelims.add(Keyword.ARRAY_BEGIN);
        ...
        for (int i = pos; i < chars.length; i++ ) {
            ...
            if (Character.isWhitespace(c) && strbuff.length() == 0) {
                ...
                if (keyw == null) {
                    ...
                    if (var != null) {
                        ...
                        i = parseFunctionInvocation(f, src, i)[0]; // first item
                        // in
                        // returned
                        // array is
                        // new
            ...
            if (data.capacity() - data.position() < THRESHOLD) {
                System.err.println("SCRIPT COMPILER: buffer threshold exceded; reallocating bytebuffer");
    ...
    /*
     * Expects the given char position to be the opening delimiter for the
     * conditional.
     */
    private int parseConditional(final String src, int pos) throws ScriptCompilationException {
        ...
        // write the whole IF statement compilation into a new buffer so we can
        ...
        while (cont) {
            ...
            pos = argEnd + 1;
            buff.put(Bytecodes.END_COND); // signal end of if condition - NOT
            // END OF BLOCK
                ...
                if (Character.isWhitespace(c) && sb.length() == 0) {
                    ...
                    if (sb.toString().equals(Keyword.ELSE.sym)) {
                        ...
                        if (argBegin < 0 || !src.substring(i, argBegin).trim().equals(Keyword.IF.sym)) {
                            ...
                            if (endPos < 0) {
                                throw (new ScriptCompilationException("reached end of block without closing delimeter",
                                        src, pos));
                            }
                            ...
                            if (!expif.equals(Keyword.IF.sym)) {
                                throw (new ScriptCompilationException(
                                        "illegal conditional delimeter: " + src.charAt(i), src, pos));
                            }
    ...
    private int parseForLoop(final String src, final int pos) throws ScriptCompilationException {
        ...
        for (int i = 0; i < chars.length - 1; i++ ) {
            ...
            if (kw != null && isForOperator(kw)) {
                ...
                case DECREM:
                    ...
                    // parseEvaluation(sb.toString(), src, npos +
                    // forpts[1].length(), Flags.TYPE_FLOAT);
                    break;
                case ADD_MOD:
    ...
    private int parseVarFromType(final Keyword keyw, final String src, final int pos) throws ScriptCompilationException {

        return parseVarFromType(keyw, false, src, pos);
    }

    private int parseVarFromType(final Keyword keyw, final boolean constant, final String src, final int pos)
            throws ScriptCompilationException {
    ...
    private int parseVarAssign(final int varType, final String name, final String src, final boolean alloc,
            final boolean constant, final int pos) throws ScriptCompilationException {
        ...
        String str = src.substring(st, end).trim(); // get the assignment
        // statement and trim
        // whitespace
    ...
    private int parseReturn(final String src, final Keyword type, final int pos) throws ScriptCompilationException {
    ...
    private void parseString(final String str, final String src, final int pos) throws ScriptCompilationException {
        ...
        if (str.startsWith(Keyword.STR_MARK.sym)) {
            ...
            for (int i = 0; i < s.length(); i++ ) {
                ...
                if (c == REF && i != 0 && s.charAt(i - 1) == ESCAPE) {
                    ...
                    if (closeInd < 0) {
                        throw (new ScriptCompilationException("mismatched variable reference in string literal", src,
                                pos));
                    }
            ...
            if (str.contains(Keyword.PARAM_BEGIN.sym) && str.contains(Keyword.PARAM_END.sym)) {
                ...
                if (f[ind].getReturnType() != Keyword.STRING) {
                    throw (new ScriptCompilationException("function in string expression must return type string", src,
                            pos));
                }
            } else {
                ...
                if (ref.varType == Flags.TYPE_STRING) {
                    ...
                    throw (new ScriptCompilationException("cannot use a non-string variable as string expression", src,
                            pos));
                }
            }
        }
     ...
     * @param pos
     * @param returnFlag
     *            flag for return type - should be any TYPE_[vartype] flags OR
     *            RETURN_FLOAT_STRICT for disallowing int upcasting
     * @throws ScriptCompilationException
     */
    private void parseEvaluation(String str, final String src, final int pos, final int returnFlag)
            throws ScriptCompilationException {
        ...
        for (int i : keys) {
            if (marks.get(i) && !strval(nstr.charAt(i)).equals(Keyword.BLOCK_BEGIN.sym)) {
                nstr.insert(i, Keyword.BLOCK_BEGIN.sym);
                    ...
                    && !strval(nstr.charAt( (i == nstr.length()) ? i - 1 : i)).equals(Keyword.BLOCK_END.sym)) {
        ...
        // System.out.println(exp + " " + Keyword.flagToTypeKey(returnFlag) +
        // " " + Keyword.flagToTypeKey(type));
        ...
        for (int i = 0; i < pts.length; i++ ) {
            ...
            if (Keyword.isOperator(s) || isBooleanOperator(s)) {
                if (isBooleanOperator(s)) {
                    if (s.equals(String.valueOf(MathRef.EQUALS))) {
                        // back to
                        // language
                        // Keyword
                        // boolean
                ...
                keyw = Keyword.getFromSymbol(s); // reload 'keyw' for any
                // converted operators
                ...
                if (s.contains(".")) {
                    if (returnFlag == Flags.TYPE_INT) {
                        throw (new ScriptCompilationException("type int cannot be assigned to floating point value",
                                src, pos));
                    }
                    double d;
                    try {
                ...
                int mpos = parseFunctionInvocation(funcs, func, argStart)[1]; // second
                // position
                // in
                // array
                // is
                 ...
                 * if(Keyword.typeKeyToFlag(rtype) != returnFlag &&
                 * !isTypeCompatible(returnFlag, rtypeFlag)) throw(new
                 * ScriptCompilationException
                 * ("function return type does not match expression", src,
                 * pos));
                 */
                ...
                /*
                 * if(v != null && v.varType != Flags.TYPE_INT && v.varType !=
                 * Flags.TYPE_FLOAT && v.varType != returnFlag) throw(new
                 * ScriptCompilationException
                 * ("variable type does not match expression: " + s, src, pos));
                 * if(v != null && returnFlag == Flags.TYPE_INT && v.varType ==
                 * Flags.TYPE_FLOAT) throw(new ScriptCompilationException(
                 * "float variable does not match expected integer expression: "
                 * + s, src, pos)); if(v != null && v.varType ==
                 * Flags.TYPE_BOOL) hasbool = true;
                 */
        ...
        /*
         * if(returnFlag == Flags.TYPE_BOOL && !hasbool) throw(new
         * ScriptCompilationException("expression does not match boolean type",
         * src, pos)); else if(returnFlag != Flags.TYPE_BOOL && hasbool)
         * throw(new ScriptCompilationException(
         * "boolean expression does not match assigned type", src, pos));
         */
    ...
    private void parseBoolean(String str, final String src, final int pos) throws ScriptCompilationException {

        // replace all two characters or special boolean operators with internal
        // stand-ins that work with the bounds of the parser
        ...
        if (str.contains(Keyword.ASSIGN.sym)) {
            throw (new ScriptCompilationException("found invalid assignment operator", src, pos
                    + str.indexOf(Keyword.ASSIGN.sym)));
        }
    ...
    private final String[] validOps = new String[] { strval(MathRef.AND_BOOL), strval(MathRef.OR_BOOL),
            strval(MathRef.EQUALS), strval(MathRef.NOT_EQUALS), strval(MathRef.LESS_EQUALS),
            strval(MathRef.LESS_EQUALS), strval(MathRef.GREAT_EQUALS) };

    // ------------------------------------------------------

    private boolean isBooleanOperator(final String s) {
    ...
    private int[] parseFunctionInvocation(final Function[] matching, final String src, final int pos)
            throws ScriptCompilationException {
    ...
    /*
     * returns int[] (UGH!) so new src position marker AND the matching
     * function's position in the given array can be returned. Curse you Java
     * for not having multiple return value support! </3
     */
    // INVOKE cmd -> long type id -> arguments
    private int[] parseFunctionInvocation(Function[] matching, final int type, final String src, final int pos)
            throws ScriptCompilationException {

        if (!String.valueOf(src.charAt(pos)).equals(Keyword.PARAM_BEGIN.sym)) {
            throw (new ScriptCompilationException("function invocation - expected argument delimeter: "
                    + src.charAt(pos), src, pos));
        }
        ...
        funcLoop:
            for (int i = 0; i < matching.length; i++ ) {
                ...
                if (!validCount && i == matching.length - 1) {
                    throw (new ScriptCompilationException("arguments do not match parameters for function " + f + " "
                        + params.length, src, pos));
                } else if (!validCount) {
                    continue;
                }
                ...
                for (int ii = 0; ii < ptypes.length; ii++ ) {
                    ...
                    try {
                        ...
                        if (i == matching.length - 1) {
                            ...
                            throw (e);
                            // throw(new
                            // ScriptCompilationException("arguments do not match function parameters for "+
                            // f, src, pos));
                        }
                        buff = curr;
                        continue funcLoop;
                ...
                if (type != 0 && rtype != type && (rtype != Flags.TYPE_INT || type != Flags.TYPE_FLOAT)) {
    ...
    private String[] splitArgs(final String argStr, final String ex) {
        ...
        for (int i = 0; i < argStr.length(); i++ ) {
            ...
            if (s.equals(Keyword.PARAM_BEGIN.sym) || s.equals("[")) {
                paramDelims++ ;
            } else if (s.equals(Keyword.PARAM_END.sym) || s.equals("]")) {
                paramDelims-- ;
    ...
    private Variable putVarRef(final String s, final ByteBuffer buff, final String src, final int pos)
            throws ScriptCompilationException {
    ...
    private Variable putVarRef(final String s, final ByteBuffer buff, final String src, final int pos, final int type)
            throws ScriptCompilationException {
    ...
    private boolean isDigitValid(final int len, final boolean shouldPermit) {

        return len > 0 && shouldPermit;
    }

    private void checkValidName(final String name) throws ScriptCompilationException {

        for (Keyword k : Keyword.values()) {
            if (k.sym.equals(name)) {
                throw (new ScriptCompilationException("illegal member string identifier: " + name
    ...
    /*
     * Takes an expression returned by MathRef.shuntingYard and parses the type
     * of its return value
     */
    private int parseExpressionType(final String exp, final String src, final int pos)
            throws ScriptCompilationException {
        ...
        for (int i = 0; i < pts.length; i++ ) {
            String chk = MathRef.matchPlaceholderKeyword(pts[i].charAt(0));
            if (Keyword.isOperator(pts[i]) || chk != null) {
                String s = (chk != null) ? chk : pts[i];
                int opRetFlag = Keyword.getFromSymbol(s).getReturnType();
                if (opRetFlag == Flags.RETURN_BOOL) {
                    ...
                    if (typeQueue.getFirst() == Flags.TYPE_FLOAT || typeQueue.getLast() == Flags.TYPE_FLOAT) {
                        ...
                        retType = Flags.TYPE_INT;
                            ...
                            && Keyword.getFromSymbol(pts[i]) == Keyword.MULTIPLY) {
                ...
                for (Function f : funcMatches) { // we need to check for each
                    // function
                    ...
                    for (int n = 0; n < args.length; n++ ) { // check each
                        // argument
                        String s = args[n];
                        if (!s.startsWith(Keyword.STR_MARK.sym)) {
                            s = parser.shuntingYard(s);
                        }
                        int type = parseExpressionType(s, src, pos); // recursive
                        // call for
                        // expression
                        // return
                        // type
            ...
            } else if (pts[i].equals(Keyword.TRUE.sym) || pts[i].equals(Keyword.FALSE.sym)) {
                typeQueue.push(Flags.TYPE_BOOL);
                ...
                if (var == null) {
                    // int type =
                    // parseExpressionType(parser.shuntingYard(pts[i]), src,
                    // pos);
    ...
    private boolean isTypeCompatible(final int type0, final int type1) {
    ...
    private boolean isVector(final String s) {
    ...
    private int getVarTypeFromKeyword(final Keyword keyw) {
    ...
    private boolean isForOperator(final Keyword k) {
    ...
    private void clear(final StringBuilder sb) {

        sb.delete(0, sb.length());
    }

    private String strval(final char c) {

        return String.valueOf(c);
    }

    private int findParamEnd(final String src, final int pos) throws ScriptCompilationException {

        if (!strval(src.charAt(pos)).equals(Keyword.PARAM_BEGIN.sym)) {
            throw (new ScriptCompilationException("invalid starting position for end param: found=" + src.charAt(pos),
                    src, pos));
        }
        int i, init = 0, end = 0;
    ...
    private void printWarning(final Function context, final String msg) {
    ...
    final class Variable {
        ...
        Variable(final String name, final int varType) {
