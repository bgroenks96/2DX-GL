...
/**
 * Implementation of World2D for use with OpenGL coordinate system. This
 * implementation removes World2D's y-axis inversion and does not round Point
 * conversions between world space and screen space since OpenGL accepts float
 * and double values for vertex coordinates. Use of this class is entirely
 * optional with OpenGL since the orthographic projection matrix can take care
 * of the same transformation between world-space and the screen viewport.
 * 
...
public final class GLWorld2D extends World2D {
    ...
    /*
     * Note that ALL uses of variable 'minY' are in this implementation actually
     * the maximum Y but the same variable from World2D's inverted Y axis screen
     * space has to be used.
     * 
     * Yes this is ridiculously counter-intuitive and poor foresight in design.
     * I'm sorry :(
     */
     ...
     * @param ppu
     */
    public GLWorld2D(final double minX, final double minY, final int viewWidth, final int viewHeight, final float ppu) {

        super(minX, minY, viewWidth, viewHeight, ppu);
    }

    @Override
    public void setLocation(final double minX, final double minY) {
    ...
    @Override
    public void setViewSize(final int viewWt, final int viewHt, final float ppu) {
    ...
    private class RectangleDouble extends Rectangle {
        ...
        RectangleDouble(final double x, final double y, final double wt, final double ht) {
