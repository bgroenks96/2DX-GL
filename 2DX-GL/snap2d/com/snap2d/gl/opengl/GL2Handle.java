...
public class GL2Handle implements GLHandle {
	...
	public void draw2d(int buffId) {
		...
		if(buffObj.textured) {
			gl.glEnableClientState(GL2.GL_TEXTURE_COORD_ARRAY);
			checkGLError("glEnableClientState [compat]");
			gl.glVertexPointer(2, GL2.GL_DOUBLE, 4 * Buffers.SIZEOF_DOUBLE, 0);
			checkGLError("glVertexPointer [compat]");
			gl.glTexCoordPointer(2, GL2.GL_DOUBLE, 4 * Buffers.SIZEOF_DOUBLE, 2 * Buffers.SIZEOF_DOUBLE);
			checkGLError("glTexCoordPointer [compat]");
		} else {
			gl.glVertexPointer(2, GL2.GL_DOUBLE, 6 * Buffers.SIZEOF_DOUBLE, 0);
			checkGLError("glVertexPointer [compat]");
			gl.glEnableClientState(GL2.GL_COLOR_ARRAY);
			checkGLError("glEnableClientState [compat]");
			gl.glColorPointer(4, GL2.GL_DOUBLE, 6 * Buffers.SIZEOF_DOUBLE, 2 * Buffers.SIZEOF_DOUBLE);
	...
	public void putQuad2f(int rectBuffId, float x, float y, float wt, float ht, FloatBuffer colorBuffer) {
		...
		if (buffObj.storeHint == BufferUsage.STATIC_DRAW) {
			...
			//gl.glBufferSubData(GL.GL_ARRAY_BUFFER, buffpos, buffSize - buffpos, floatBuff);
	...
	private BufferObject initVBO2f(BufferUsage storeType, GeomFunc drawFunc, int nverts, 
		...
		BufferObject buffObj = new BufferObject(vboId[0], nverts, nobjs, Buffers.SIZEOF_FLOAT, buffSize, 
	...
	private BufferObject initVBO2d(BufferUsage storeType, GeomFunc drawFunc, int nverts, 
		...
		BufferObject buffObj = new BufferObject(vboId[0], nverts, nobjs, Buffers.SIZEOF_DOUBLE, buffSize, 
				textured, drawFunc, storeType);
		if(buffObj.storeHint != BufferUsage.STATIC_DRAW)
	...
	private class BufferObject {
		...
		BufferObject(int vboId, int nverts, int nobjs, int typeSize, int size, boolean textured, 
				GeomFunc drawFunc, BufferUsage usage) {
			this.vbo = vboId; this.nobjs = nobjs;
			this.size = size;
