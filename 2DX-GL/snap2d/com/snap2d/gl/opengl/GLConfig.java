 ...
 * OpenGL rendering engine (powered by JOGL 2.0).  GLConfig loads default values
 * from existing Java system property values (creating them if they have not yet been set)
 * and stores its current configuration internally in a map.  <b>This means GLConfig will
 * default to whatever values are currently set via System.getProperty/setProperty.</b>
 * You will need to explicitly call {@link #set(Property, String)} on the GLConfig to
 * change any values that have been modified via system properties (i.e. the System
 * class calls or through a VM -D property value flag).
...
public class GLConfig {
	...
	/**
	 * Sets the system configuration property for Snap2D/OpenGL graphics rendering.<br/>
	 * 
	 * @param property
	 *            Property to set
	 * @param value
	...
	public enum Property {
		...
		/**
		 * String (default=max)
		 * Specify which OpenGL version profile Snap2D should attempt to load when initializing
		 * the renderer.  "max" or "min" can be specified to favor the highest or lowest
		 * available GL rendering profile respectively.  You may also specify a particular
		 * profile by name, although whether or not it is used depends on what is provided by
		 * the user's underlying hardware.
		 * Values for common GL versions:
		 * OpenGL 2.1-3.0 = "GL2"
		 * OpenGL 3.1-3.3 = "GL3"
		 * OpenGL 4.0-4.4 = "GL4"
		 * OpenGL ES 1.0  = "GL2ES1"
		 * OpenGL ES 2.0  = "GL2ES2"
		 * Values for other JOGL profiles can be found under JOGL's documentation, although it is
		 * not guaranteed that Snap2D will function correctly with all of them.
		 * @see <a href="https://jogamp.org/deployment/jogamp-next/javadoc/jogl/javadoc/constant-values.html#javax.media.opengl.GLProfile.GL2">JOGL GLProfile Constant Values</a>
		 */
		GL_PROFILE("snap2d.gl.opengl.use_profile", GL_PROFILE_HIGHEST),
		
		/**
		 * Boolean (default=false if OpenGL core profile available, true otherwise)
		 * <br/><br/>
	...
	public static class GLConfigException extends GLException {
		...
		public GLConfigException(String msg) {super(msg);}
		public GLConfigException(String msg, Throwable t) {super(msg, t);}
	}
	 ...
	 * user configuration.
	 */
	static GLProfile loadGLProfile(GLConfig config) throws GLConfigException {
