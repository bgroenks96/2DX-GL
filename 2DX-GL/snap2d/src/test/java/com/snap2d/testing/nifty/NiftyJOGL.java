...
public class NiftyJOGL {
    ...
    class RenderTest implements GLEventListener {
        ...
        public void display(final GLAutoDrawable arg0) {
            ...
            gl.glMultiDrawArrays(GL.GL_TRIANGLE_STRIP, new int[] { 0, 4 }, 0, new int[] { 4, 4 }, 0, 2);
            gl.glUseProgram(0);
        ...
        public void init(final GLAutoDrawable arg0) {
            ...
            gl.glBufferData(GL.GL_ARRAY_BUFFER, 48 * Buffers.SIZEOF_FLOAT, null, GL.GL_STATIC_DRAW);
            ...
            floatBuff.put(coords[0]);
            floatBuff.put(coords[1]);
            floatBuff.put(new float[] { 1, 0, 0, 1 });
            floatBuff.put(coords[2]);
            floatBuff.put(coords[3]);
            floatBuff.put(new float[] { 0, 1, 0, 1 });
            floatBuff.put(coords[4]);
            floatBuff.put(coords[5]);
            floatBuff.put(new float[] { 0, 0, 1, 1 });
            floatBuff.put(coords[6]);
            floatBuff.put(coords[7]);
            floatBuff.put(new float[] { 1, 0, 1, 1 });

            floatBuff.put(coords2[0]);
            floatBuff.put(coords2[1]);
            floatBuff.put(new float[] { 1, 0, 1, 1 });
            floatBuff.put(coords2[2]);
            floatBuff.put(coords2[3]);
            floatBuff.put(new float[] { 1, 1, 0, 1 });
            floatBuff.put(coords2[4]);
            floatBuff.put(coords2[5]);
            floatBuff.put(new float[] { 0, 1, 0, 1 });
            floatBuff.put(coords2[6]);
            floatBuff.put(coords2[7]);
        ...
        private void loadShaders(final GL2GL3 gl) {
            ...
            try {
                ...
                int vs = compileShader(gl, GL2ES2.GL_VERTEX_SHADER, src);
                src = read(ClassLoader.getSystemClassLoader().getResource(DEFAULT_FS));
                int fs = compileShader(gl, GL2ES2.GL_FRAGMENT_SHADER, src);
        ...
        private int compileShader(final GL2GL3 gl, final int type, final String src) throws Exception {
            ...
            gl.glGetShaderiv(sobj, GL2ES2.GL_COMPILE_STATUS, compiled, 0);

            if (compiled[0] == GL.GL_FALSE) {
                int[] logLength = new int[1];
                gl.glGetShaderiv(sobj, GL2ES2.GL_INFO_LOG_LENGTH, logLength, 0);
        ...
        private boolean linkProgram(final GL2GL3 gl) {
            ...
            gl.glGetProgramiv(prog, GL2ES2.GL_LINK_STATUS, intBuff);
