...
/**
 * Convert awt key codes to nifty key codes, based on lwjgl <a
 * href="http://java-game-lib.svn.sourceforge
 * .net/viewvc/java-game-lib/trunk/LWJGL
 * /src/java/org/lwjgl/opengl/KeyboardEventQueue
 * .java?revision=3116&content-type=text%2Fplain" >code</a>.
 * 
 * @author acoppes
 */
class NewtToNiftyKeyCodeConverter {

    public int convertToNiftyKeyCode(final int key_code) {

        // manually map positioned keys
        // Note: currently JOGL does not separate between left and right keys
        // here so we map everything to left for now
        switch (key_code) {
