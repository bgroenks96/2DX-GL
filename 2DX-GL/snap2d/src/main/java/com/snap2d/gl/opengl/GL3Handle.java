...
public class GL3Handle implements GLHandle {
    ...
    GL3Handle(final GLConfig config) {
     ...
     * orthographic projection matrix that transforms vertices 2D space. The
     ...
     * @param y
     * @param width
     *            the unit width of the viewport
     * @param height
     *            the unit height of the viewport
     * @param ppu
    ...
    public void addLightSource(final LightSource light) {

        lights.add(light);
    }

    public void removeLightSource(final LightSource light) {
    ...
    public void setAmbientLightFactor(final float ambientFactor) {

        this.ambientFactor = ambientFactor;
    }

    public void setAmbientLightColor(final float[] ambientLightColor) {
    ...
    public void updateLightData() throws IllegalStateException {
        ...
        for (LightSource light : lights) {
            ...
            lightCoords.put(loc.getFloatX());
            lightCoords.put(loc.getFloatY());
    ...
    private BufferObject initVBO2d(final BufferUsage storeType,
        ...
        int buffSize;
        if (textured) {
            gl.glVertexAttribPointer(vertCoordPos, 2, GL2GL3.GL_DOUBLE, false, 4 * Buffers.SIZEOF_DOUBLE, 0);
            checkGLError("glVertexAttribPointer");
            gl.glEnableVertexAttribArray(texCoordPos);
            checkGLError("glEnableVertexAttribArray");
            gl.glVertexAttribPointer(texCoordPos,
            ...
            gl.glVertexAttribPointer(vertCoordPos, 2, GL2GL3.GL_DOUBLE, false, 6 * Buffers.SIZEOF_DOUBLE, 0);
            checkGLError("glVertexAttribPointer");
            gl.glEnableVertexAttribArray(vertColorPos);
            checkGLError("glEnableVertexAttribArray");
            gl.glVertexAttribPointer(vertColorPos,
    ...
    /*
     * @Deprecated public void drawRect2f(float x, float y, float wt, float ht)
     * { GL2 gl = GLContext.getCurrentGL().getGL2(); if(texBound && texEnabled)
     * { setColor4f(1,1,1,1); if(texCoords.length < 8) // 4 corners * 2
     * coordinates per corner throw(new GLException("too few tex coords: " +
     * texCoords.length + " < 8")); } int tcoord = 0;
     * gl.glBegin(GL_TRIANGLE_STRIP); if(texBound && texEnabled)
     * gl.glTexCoord2f(texCoords[tcoord++], texCoords[tcoord++]);
     * gl.glVertex2f(x, y); if(texBound && texEnabled)
     * gl.glTexCoord2f(texCoords[tcoord++], texCoords[tcoord++]);
     * gl.glVertex2f(x, y + ht); if(texBound && texEnabled)
     * gl.glTexCoord2f(texCoords[tcoord++], texCoords[tcoord++]);
     * gl.glVertex2f(x + wt, y); if(texBound && texEnabled)
     * gl.glTexCoord2f(texCoords[tcoord++], texCoords[tcoord++]);
     * gl.glVertex2f(x + wt, y + ht); gl.glEnd(); }
     */
    /*
     * public void drawRect2d(double x, double y, double wt, double ht) {
     * if(texBound && texEnabled) { setColor3f(1,1,1); if(texCoords.length < 8)
     * // 4 corners * 2 coordinates per corner throw(new
     * GLException("too few tex coords: " + texCoords.length + " < 8")); } int
     * tcoord = 0; gl.glBegin(GL_TRIANGLE_STRIP); if(texBound && texEnabled)
     * gl.glTexCoord2f(texCoords[tcoord++], texCoords[tcoord++]);
     * gl.glVertex2d(x, y); if(texBound && texEnabled)
     * gl.glTexCoord2f(texCoords[tcoord++], texCoords[tcoord++]);
     * gl.glVertex2d(x, y + ht); if(texBound && texEnabled)
     * gl.glTexCoord2f(texCoords[tcoord++], texCoords[tcoord++]);
     * gl.glVertex2d(x + wt, y + ht); if(texBound && texEnabled)
     * gl.glTexCoord2f(texCoords[tcoord++], texCoords[tcoord++]);
     * gl.glVertex2d(x + wt, y); if(texBound && texEnabled)
     * gl.glTexCoord2f(texCoords[tcoord++], texCoords[tcoord++]);
     * gl.glVertex2d(x, y); gl.glEnd(); }
     */

    /*
     * private void drawNoGLSL2f(BufferObject buffObj) { gl.glEnableClientState(
     * GL2.GL_VERTEX_ARRAY); if(texBound && texEnabled) {
     * gl.glEnableClientState(GL2.GL_TEXTURE_COORD_ARRAY); gl.glVertexPointer(2,
