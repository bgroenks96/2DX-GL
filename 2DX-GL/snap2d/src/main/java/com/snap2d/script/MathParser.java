...
public class MathParser {
    ...
    /**
     * Parses a mathematical String expression and returns the result as an
     * Operand. Result may be a scalar or vector depending on expression.
     * 
     * @param input
     * @return the result as a VecMath.Operand type (sub-type Scalar or Vec).
     * @throws MathParseException
     */
    public Operand parse(final String input) throws MathParseException {
    ...
    /**
     * Parses a mathematical String expression and returns the result as a
     * double. All operands in the expression must be scalars. For vector
     * support, use {@link #parse(String)}
     * 
     * @param input
     * @return the result as a double value
     * @throws MathParseException
     */
    public double parseScalars(final String input) throws MathParseException {
    ...
    protected void format(final StringBuilder sb) {
        ...
        if (mismatch) {
            System.out.println("MathParser: Assuming mismatched delimeter positions: " + sb.toString());
        }
    }
    ...
    protected String findNegatives(final String input) {
        ...
        for (int i = 0; i < sb.length(); i++ ) {
            ...
            if (sb.charAt(i) == neg && isNumber(sb.charAt(i + 1))) {
                if (MathRef.isOperator(sb.charAt(i - 1)) || isNonClosingDelimiter(sb.charAt(i - 1))) {
                    sb.insert(i, "(0");
                    sb.insert(i + 4, ')'); // advance 2 chars +2 more for the
                    // new characters we just added
                }
            }
        }
        return sb.toString();
    ...
    protected String shuntingYard(final String input) throws MathParseException {
        ...
        for (int i = 0; i < chars.length; i++ ) {
            ...
            if (MathRef.isOperator(c)) {
                ...
                while (peek != null && peek != '(' && peek != ')' && MathRef.opPreceeds(peek, c)) {
    ...
    /*
     * Calculates the result of the String 'input' result of
     * shuntingYard(String) Supports all math operations defined in MathRef and
     * VecMath
     */
    protected Operand calculate(final String input) throws MathParseException {
        ...
        for (int i = 0; i < chars.length; i++ ) {
                ...
                // negatives
            {
                ...
                if (num.length() > 0) {
                    if (num.toString().startsWith("[")) {
                        ...
                        try {
                            ...
                            // e.printStackTrace();
                            throw (new MathParseException("error parsing input: " + num.toString()));
                        }
                    }
                }
                ...
                if (numStack.size() < 2) {
                    ...
                    throw (new MathParseException("error parsing input: too few operands"));
                }
        ...
        if (numStack.size() != 1) {
            throw (new MathParseException("error parsing input: operand evaluation did not complete properly"));
        }
    ...
    private String checkDelimiters(final String input) {
     ...
     */
    public boolean isTypedChar(final char c) {
    ...
    public boolean isOpeningDelimiter(final char c) {

        return c == '(' || c == '[';
    }

    public boolean isNonClosingDelimiter(final char c) {

        return isDelimiter(c) && c != ']' && c != ')';
    }

    public boolean isNonOpeningDelimter(final char c) {

        return isDelimiter(c) && c != '(' && c != '[';
    }

    public boolean isBrace(final char c) {

        return c == BRACE_OPEN || c == BRACE_CLOSE;
    }

    public int findBraceClose(final String input, final int pos) {
    ...
    /*
     * Calculates the result of the String 'input' result of
     * shuntingYard(String) Old version - no vector support
     */
