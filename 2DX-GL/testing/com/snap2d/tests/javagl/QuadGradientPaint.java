...
public class QuadGradientPaint implements Paint {
	...
	public QuadGradientPaint(
			final de.lessvoid.nifty.tools.Color c0,
	...
	private class QuadGradientPaintContext implements PaintContext {
		...
		private Raster createRaster(int x, int y, int w, int h) {
			WritableRaster raster = cm.createCompatibleWritableRaster(w, h);
			//final double radius = 1.5*Point.distance(0, 0, userBounds.getCenterX(), userBounds.getCenterY());
		...
		private float interpolateComponentHSB(float c0, float c1, float c2, float c3, float x, float y, float w, float h) {
	...
	private static Color convertNiftyColorToAwt(de.lessvoid.nifty.tools.Color niftyColor) {
