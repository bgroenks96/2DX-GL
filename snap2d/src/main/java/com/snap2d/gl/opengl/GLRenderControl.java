...
public class GLRenderControl implements RenderController, GLEventListener {
    ...
    GLRenderControl(final GLWindow glWin, final GLConfig config) {
    ...
    private volatile GLRenderable[] renderables = new GLRenderable[0]; // independent
    // of
    // task
    // list
    ...
    private void checkAddQueue() {
        ...
        if (addQueue.size() > 0) {
            ...
            renderables = rtasks.toArray(new GLRenderable[rtasks.size()]);
        }
    }
     ...
     */
    @Override
    public void reshape(final GLAutoDrawable arg0, final int x, final int y, final int width, final int height) {
        ...
        // fbo.reset(gl, wt, ht,
        // glWin.getChosenGLCapabilities().getNumSamples(), true);
     ...
     */
    public synchronized void addRenderable(final GLRenderable r, int pos) {
     ...
     */
    public synchronized void removeRenderable(final GLRenderable r) {

        delQueue.add(r);
    }

    public synchronized boolean isRegistered(final GLRenderable r) {
    ...
    public void copyRenderablesTo(final GLRenderControl rc) {
    ...
    /**
     * Disposes of this GLRenderControl and all of its internally held resources
     * with the exception of the GLWindow object responsible for the GLContext.
     * Subsequent calls to a disposed GLRenderControl's methods will result in
     * errors. This method blocks for a maximum of five seconds or until the
     * GLRenderLoop successfully completes its shutdown operations.
     */
    public void dispose() {
    ...
    /**
     * Loop where render/update logic is executed. The loop is internally
     * synchronized with the OpenGL rendering thread. <br/>
     * <br/>
